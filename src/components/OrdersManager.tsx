        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 text-xs md:text-sm">
            ...
            <div>
              ...
              <p className="font-semibold text-gold-600">${finalTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
                ...
                <div key={index} className="bg-gray-50 rounded-lg p-3 md:p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] md:text-xs text-gray-500">
                      Quantity: {item.quantity} × ${item.price.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </p>
                  </div>
          ...
          <div className="border-t-2 border-gray-200 pt-3 md:pt-4">
            <div className="space-y-1.5 md:space-y-2 text-xs md:text-sm">
              <div className="flex justify-between">
                <span>Subtotal:</span>
                <span className="font-semibold">${order.total_price.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
              </div>
              {order.shipping_fee && order.shipping_fee > 0 && (
                <div className="flex justify-between">
                  <span>Shipping Fee:</span>
                  <span className="font-semibold">${order.shipping_fee.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                </div>
              )}
              <div className="flex justify-between text-base md:text-lg font-bold border-t-2 border-gray-200 pt-2">
                <span>Total:</span>
                <span className="text-gold-600">${finalTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
              </div>
            </div>
          </div>
